...
-- 직업 아이디별 개수와 급여의 총합계 구하기
SELECT job_id, COUNT(job_id) 개수, SUM(salary) 총합계    -- 카운트안에 *이어도 괜찮음
FROM job
GROUP BY job_id;
...
-- 직업 아이디별 개수와 급여의 최대값과 최소값 구하기
-- 내답
SELECT job_id, COUNT(*) 개수, MAX(salary) 최대급여, MIN(salary) 최소급여
FROM job
GROUP BY job_id;
-- 선생님답
